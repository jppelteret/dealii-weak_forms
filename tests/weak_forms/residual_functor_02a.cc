...
// - Linearize with respect to scalar fields
    ...
    const SubSpaceExtractorDerivativesType &subspace_extractor_lin)
{
  ...
  const auto residual = residual_functor(
    "R", "R", soln_val, soln_grad, soln_hess, soln_lap, soln_d3);
  ...
  const auto residual_functor =
    residual_view.template value<SDNumber_t, dim, spacedim>(
      [](const SDNumber_t &                u,
         ...
         const Tensor<3, dim, SDNumber_t> &d3_u) { return Result_t{}; },
      [](const SDNumber_t &                u,
         const Tensor<1, dim, SDNumber_t> &grad_u,
         const Tensor<2, dim, SDNumber_t> &hess_u,
         const SDNumber_t &                lap_u,
...
main(int argc, char **argv)
{
...

  {
    ...
    run<2>(subspace_extractor_s, subspace_extractor_s);
    run<3>(subspace_extractor_s, subspace_extractor_s);
    deallog << "OK" << std::endl;
  }

  {
    LogStream::Prefix prefix("Vector");
    run<2>(subspace_extractor_v, subspace_extractor_s);
    run<3>(subspace_extractor_v, subspace_extractor_s);
    deallog << "OK" << std::endl;
  }

  {
    LogStream::Prefix prefix("Tensor");
    run<2>(subspace_extractor_T, subspace_extractor_s);
    run<3>(subspace_extractor_T, subspace_extractor_s);
    deallog << "OK" << std::endl;
  }
