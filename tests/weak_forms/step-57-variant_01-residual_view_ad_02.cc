...
#  pragma GCC diagnostic ignored "-Wunused-lambda-capture"
  ...
  Step57<dim>::assemble(const bool initial_step, const bool assemble_matrix)
  {
    ...
    // Residual
    const auto residual_func_v      = residual_functor("R", "R", v, grad_v);
    const auto residual_func_div_v  = residual_functor("R", "R", div_v, p);
    const auto residual_func_grad_v = residual_functor("R", "R", grad_v);
    const auto residual_func_p      = residual_functor("R", "R", div_v);

    const auto residual_ss_v      = residual_func_v[test_v];
    const auto residual_ss_div_v  = residual_func_div_v[div_test_v];
    const auto residual_ss_grad_v = residual_func_grad_v[grad_test_v];
