...
namespace Step57
{
  ...
  Step57<dim>::assemble(const bool initial_step, const bool assemble_matrix)
  {
    ...
    assembler += bilinear_form(grad_test_v, viscosity, grad_trial_v).dV() +
                 ...
                 bilinear_form(test_v, 1, grad_trial_v * v).dV() -
    ...
    assembler -= -linear_form(grad_test_v, viscosity * grad_v).dV() -
                 linear_form(test_v, grad_v * v).dV() +
                 linear_form(div_test_v, p).dV() +
                 linear_form(test_p, div_v).dV() -
                 linear_form(div_test_v, gamma * div_v).dV();
