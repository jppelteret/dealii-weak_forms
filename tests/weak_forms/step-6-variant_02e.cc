...
//
// Uses extractors with conversion from the field solution.
...
Step6<dim>::assemble_system()
{
  ...
  assembler +=
    bilinear_form(test_function(field_solution[subspace_extractor].gradient()),
                  mat_coeff.value(coefficient),
                  trial_solution(field_solution[subspace_extractor].gradient()))
      .dV() -
    linear_form(test_function(field_solution[subspace_extractor].value()),
                rhs_coeff.value(rhs))
      .dV();
