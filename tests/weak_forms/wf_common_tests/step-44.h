...
namespace Step44
{
  ...
  public:
    ...
    // === Required for differentiation via energy functional form ===
    ...
    // === Required for differentiation via residual form ===
  ...
  public:
    ...
    get_P(const Tensor<2, dim, NumberType> &F, const NumberType &p_tilde) const
    {
      return F * Tensor<2, dim, NumberType>(get_S(F, p_tilde));
    }

    template <typename NumberType>
    SymmetricTensor<2, dim, NumberType>
    get_S(const Tensor<2, dim, NumberType> &F, const NumberType &p_tilde) const
    {
      return Physics::Transformations::Contravariant::pull_back(
        get_tau(F, p_tilde), F);
    }
    ...
    get_C_inv() const
    {
      return invert(symmetrize(transpose(get_F()) * get_F()));
    }
    double
    get_p_tilde() const
    {
