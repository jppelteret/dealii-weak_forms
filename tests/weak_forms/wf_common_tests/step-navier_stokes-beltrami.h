...
private:
  ...
  Vector<unsigned int> compcounter;
};
...
template <int dim>
void
NavierStokesProblem<dim>::make_grid_and_dofs()
{
...
NavierStokesProblem<dim>::assemble_system()
{
  ...
  // function
  // <code>NavierStokesProblem::assemble_system_interval</code>
      ...
      case Parallel:
        {
          ...
          for (unsigned int thread = 0; thread < n_threads; ++thread)
            threads += Threads::spawn(
              *this, &NavierStokesProblem<dim>::assemble_system_interval)(
...
template <int dim>
void
NavierStokesProblem<dim>::assemble_system_interval(
  const typename DoFHandler<dim>::active_cell_iterator &beginc,
  const typename DoFHandler<dim>::active_cell_iterator &endc)
{
  ...
  std::vector<std::vector<double>> gradT_phi_u(dofs_per_cell,
                                               std::vector<double>(n_q_points));
  std::vector<std::vector<double>> div_phi_u_p(dofs_per_cell,
                                               std::vector<double>(n_q_points));
  std::vector<std::vector<double>> residual_phi(
    dofs_per_cell, std::vector<double>(n_q_points));
  std::vector<std::vector<Tensor<1, dim>>> stab_grad_phi(
          ...
          switch (preconditioner_type)
            {
              ...
              case Vanka: /* Vanka preconditioner: expensive, bad quality */
                {
                  Timer             computing_timer2;
                  std::vector<bool> signature(3, false);
                  signature[dim] = true;
...
template <int dim>
void
NavierStokesProblem<dim>::compute_errors() const
{
...
template <int dim>
void
NavierStokesProblem<dim>::output_results(
...
template <int dim>
void
NavierStokesProblem<dim>::print_computing_times() const
{
...
// // the respective number to <code><2></code>
// // or <code><3></code>!) and
